...
function inputText() {
	...
	if (document.form1.latin.value != "no") {
		...
		var back = 0;
		var old_l;
		while ((mx = /mx([\dA-Fa-f]{4})/.exec(s)) !== null) {
			old_l = s.length;
			s = s.replace(mx[0], String.fromCodePoint(parseInt(mx[1], 16)));
			back += old_l - s.length;
		}
		while ((mx = /mxs([\dA-Fa-f]{5})/.exec(s)) !== null) {
			old_l = s.length;
			s = s.replace(mx[0], String.fromCodePoint(parseInt(mx[1], 16)));
			back += old_l - s.length;
		}
		while ((mx = /mxg([\dA-Fa-f]{4})/.exec(s)) !== null) {
			old_l = s.length;
			s = s.replace(mx[0], String.fromCodePoint(parseInt(`10${mx[1]}`, 16)));
			back += old_l - s.length;
		}
...
function insertRuby() {
	if (text.selectionStart == text.selectionEnd) {
		var kanjxi = prompt("Body");
		if (kanjxi == null || kanjxi == "") return;
		var ruby = prompt("Top (Empty for no Top)");
		if (ruby == null) return;
		var boucxuu = prompt("Bottom (Empty for no Bottom)");
...
function makeBitmap() {
	...
	if (!(/^\d+$/.test(e_rows.value))) {
		e_error.textContent += "Set Row even from 2 to 32.";
		error = true;
	}
	...
	if (!(/^\d+$/.test(e_cols.value))) {
		e_error.textContent += "Set Column even from 2 to 32.";
		error = true;
	}
